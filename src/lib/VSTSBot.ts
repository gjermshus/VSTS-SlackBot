...
import {
    ISlackAttachment,
    ISlackAttachments
} from '../interfaces/ISlackAttachment';
...
export class VSTSBot extends Bot {
    ...
    Run(): void {
        ...
        super.on('close', this.onClose);
    }

    onClose(arg0: any, arg1: any): void {
        Log('Connection closed');
        // Close the whole program, Docker should restart the container automaticlly
        process.exit(1);
    }
    ...
    private onStart(): void {
        this.user = super
            .getUsers()
            ._value.members.filter(f => f.name.toLowerCase() === this.name)[0];
        if (this.user === undefined) {
            throw new Error(
                'Bot user not found in Slack team directory, have you spelled my name right?'
            );
        }
        return;
    }
    ...
    private async onMessage(message: ISlackMessage): Promise<void> {
        if (this.shouldProcess(message)) {
            ...
            let attachments: Array<ISlackAttachment> = new Array<
                ISlackAttachment
            >();
            ...
            let attachmentsContainer: ISlackAttachments = {
                attachments: attachments
            };
            Log('Posting message to channel ' + message.channel);
            super.postMessage(message.channel, '', attachmentsContainer);
        } else if (this.isEffortMessage(message.text)) {
            let wit: Array<number> = this.extractWitId(message.text);
            for (let w of wit){
                let item: WorkItem = await this.vsts.GetWorkItem(w);
                console.log(item);                
            }
        }
    ...
    private shouldProcess(message: ISlackMessage): boolean {
        return (
            this.isChatMessage(message) &&
            (this.isChannelConversation(message) ||
                this.isPrivateMessage(message) ||
                this.isGroupMessage(message)) &&
            !this.isFromBot(message) &&
            this.isVSTSMessage(message.text)
        );
    }

    private isEffortMessage(message: string): boolean {
        let test: boolean = /effort #(\d+)/.test(message);
        return test;
    }
    ...
    private isPrivateMessage(message: ISlackMessage): boolean {
        return (
            typeof message.channel === 'string' && message.channel[0] === 'D'
        );
    }

    private isGroupMessage(message: ISlackMessage): boolean {
        return (
            typeof message.channel === 'string' && message.channel[0] === 'G'
        );
    }
}
